...
pub struct PipInstallArgs {
    ...
    ///
    /// Only applies when `--upgrade` is also provided. The strategy is restricted to named
    /// packages listed on the command line; requirements specified by URL, path, or in a
    /// requirements file are not affected.
    ///
